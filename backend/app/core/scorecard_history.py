...
def _team_for(dataset: models.Dataset) -> str | None:
    # Prefer the governance `team` column on knowledge so history buckets line up
    # with the live scorecard rollups; fall back to exploration metadata.
    knowledge = dataset.knowledge
    if knowledge is not None:
        team = getattr(knowledge, "team", None)
...
def _domain_for(dataset: models.Dataset) -> str | None:
    # Prefer the governance `domain` column; fall back to schema name.
    knowledge = dataset.knowledge
    if knowledge is not None:
        domain = getattr(knowledge, "domain", None)
...
def _snapshot_payloads(
    ...
    *,
    ...
    for dataset_score in scores:
        ...
        domain = _clean_key(_domain_for(dataset), "default")
        grouped[("domain", domain)].append(dataset_score)
        labels[("domain", domain)] = "Default domain" if domain == "default" else domain
